...
**Fetch missing songs** does not use the list. It walks `archive.json` for photo posts with images on disk and no file in `audio/`, opens each one's own page in the background tab, and takes the item struct out of the page's server-rendered state blob — no request to sign, no cursor, no dependence on how deep the list is served. That also answers the other half of the problem: a track `item_list` names but sends no `playUrl` for is routinely linked on the post's own page, so posts already written off as `noAudio` are retried rather than skipped.

It costs a page load per post, which is why it is a button and not part of a sync. Posts are opened newest-liked first, **Stop** ends it between posts, and it leaves the tab back on your profile so the next sync can reuse it. Anything still songless afterwards is recorded under `noAudio` with which of the two reasons it was.
...
`src/dev/postdetail.html` runs `hook.js` and `collector.js` against a faked post page, which is how the song pass gets a track the list wouldn't give up — see [Fetch missing songs](#fetch-missing-songs). Both failures here are silent ones. The archive page navigates the tab and asks as soon as the load reports complete, but an SPA navigation replaces the state blob a beat after that, so reading it once returns the *previous* post — and filing one post's song under another's id produces an ordinary-looking archive with the wrong music in it. The id is therefore checked and a mismatch waited out, which is asserted by swapping the blob mid-request. The other half is the reverse direction: a detail response goes through the same `handle()` as a page of the list but is not one, and adopting it would end a harvest on the spot and put a post nobody scrolled past into the run's like order. Open `http://127.0.0.1:8777/src/dev/postdetail.html`; two of its cases wait out a real timeout, so it takes about half a minute.
