...
## Install (Edge, or Chrome 111+)

1. Open `edge://extensions` (or `chrome://extensions`).
2. Turn on **Developer mode**.
3. **Load unpacked** → select this folder (`tiktok-downloader`).
4. Pin the extension and click it → **Open archive**.

Firefox is not supported: it has no File System Access API, so it cannot write into an arbitrary folder like `C:\Users\you\Videos\TikTok likes`.

## Use

1. On the archive page, click **Choose folder…** and pick your archive folder. Chromium asks for read/write permission once per browser session.
2. If it's an existing myfaveTT folder, you'll see a note saying so — its metadata is imported and its `data/.appdata/` files are left strictly alone.
3. Enter your TikTok username and press **Sync likes**.
4. A TikTok tab opens on your profile, switches to the **Liked** tab, and scrolls itself. **Leave that tab open and visible** — Chromium throttles timers in background tabs and the scroll stalls.
...
| `src/background.js` | Relay between content scripts and the archive page; manages the TikTok tab. |
| `src/archive/archive.js` | Owns the folder handle, the sync run, and all disk I/O. |
| `src/archive/viewer.js` | Offline library browser — search, sort, playback from disk. |
| `src/lib/fs.js` | File System Access wrapper, folder layout, write retries. |
...
Checked on 2026-08-02 by loading this extension into Edge with `--load-extension` and downloading from the extension's own origin — the same conditions a real sync runs under.
...
Then open `http://127.0.0.1:8777/_syntaxcheck.html`. It is dev-only and not referenced by the extension.
